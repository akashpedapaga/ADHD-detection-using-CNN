'''
#FULLY FUNCTIONAL SHAP XAI CODE
import pandas as pd
import numpy as np
...
# If shap_values is a list (for multi-class models), select the first class's SHAP values
if isinstance(shap_values, list):
    shap_values = np.array(shap_values[0])

# Ensure shap_values is 2-dimensional (samples, features) and average across samples
shap_values_mean = np.mean(np.abs(shap_values), axis=0)

# SHAP feature importance
shap_importance_df = pd.DataFrame({
    'Feature': [f'Feature {i}' for i in range(X_test_sample.shape[1])],
    'Importance': shap_values_mean.flatten()  # Ensure this is 1-dimensional
})
...
# Plot SHAP summary plot with adjusted scale for visibility
