#This file is the fully functional running code with Reduced Datasets (Both IEEE and ADHD datasets with reduced size) - UNBIASED MODEL
#ACCURACY : 0.99933
import pandas as pd
import numpy as np
...
# Load data
adhd_data1 = pd.read_csv('ADHD1.csv', header=None)
control_data1 = pd.read_csv('Control1.csv', header=None)
adhd_data2 = pd.read_csv('ADHD2.csv', header=None)
control_data2 = pd.read_csv('Control2.csv', header=None)
...
# Define the CNN model
model = Sequential()
model.add(Conv1D(filters=32, kernel_size=5, activation='relu', input_shape=(X_train_res.shape[1], 1)))
model.add(MaxPooling1D(pool_size=2))
...
# Compile the model with focal loss
model.compile(optimizer=Adam(learning_rate=0.0001), loss=focal_loss(gamma=2., alpha=0.25), metrics=['accuracy'])
